...
def test_identifier_to_class() -> None:
    with pytest.raises(pysubs2.exceptions.UnknownFormatIdentifierError) as exc_info:
...
def test_ext_to_identifier() -> None:
    with pytest.raises(pysubs2.exceptions.UnknownFileExtensionError) as exc_info:
...
def test_format_detection_fail() -> None:
    with pytest.raises(pysubs2.exceptions.FormatAutodetectionError) as exc_info:
...
def test_97_format_detection_multiprocessing_error() -> None:
    ...
    with multiprocessing.Pool(4) as pool:
        ...
        with pytest.raises(pysubs2.exceptions.FormatAutodetectionError):
